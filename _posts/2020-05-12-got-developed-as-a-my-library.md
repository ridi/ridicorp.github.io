...
PM👨‍⚖️ : 리발자님! 리디에 오신 걸 환영합니다! 저희 리디에는 리디북스라는 한국의 전자책 서비스가 있습니다. 안드로이드, iOS, Windows, macOS를 지원하는 전자책 서비스 앱이 있으며 자체 전자책 리더인 리디페이퍼도도 출시한 걸 아시나요? 흠흠…. 소개는 이 정도만 하고, 개발을 진행하시기 전에 간단한 퀴즈를 하나 풀어보고 가는 게 좋을 것 같아서 찾아왔습니다! 아래 퀴즈를 한 번 풀어보시겠어요?
...
3. 유저: 책을 구매하거나 구매한 책을 조회할 수 있는 모델
4. 구매 이력: 유저가 특정 책을 구매하거나 대여한 이력. 대여한 경우에는 만료되는 정보가 담긴 모델
