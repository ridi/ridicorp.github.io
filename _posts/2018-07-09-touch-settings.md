...
* A. 옵션이 너무 많다 -> 선택을 망설이게 된다.
* B. 선택했을 때 결과를 예측하기 어렵다 -> 선택을 망설이게 된다.
* C. 레이블이 한 번에 이해되지 않는다 -> 설정을 찾는 것이 헷갈린다.
...
![GA_PageMoveMode_LTR&RTL](/blog/img/2018-07-09/3.GA_PageMoveMode_LTR&RTL.png){:data-action="zoom"}
...
그 결과, A 안의 경우, 이미 리디북스를 잘 알거나 고급 기능을 익히는 것에 어려움이 없는 사용자들이 선택했고, B 안의 경우, 리디북스 앱 사용 경험이 많지 않거나, 복잡한 조작을 원하지 않는 사용자들이 더 많이 선택했습니다.

즉, 일반적인 사용자에게 복잡하지 않고 직관적으로 옵션을 선택할 수 있는 B 안이 더 적절할 것이라 판단했습니다.
...
또한, 사용자 테스트(UT)를 통해 사용자를 보다 폭 넓게 이해하고 문제를 다시 한 번 해결하는 과정을 거치며, 다양한 사람들이 사용하는 서비스를 디자인한다는 것은 정해진 답이 없고, 끊임없는 가설 확인과 검증의 반복이라는 것을 깨달았습니다.
