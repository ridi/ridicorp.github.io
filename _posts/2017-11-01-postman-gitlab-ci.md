...
최근에 **데이터 팀에서 만든 [lightweight-rest-tester](/blog/2017/08/31/rest-api-testing/)**가 있었지만 아쉽게도 대안이 될 수 없었습니다. 모니터링을 목적으로 만들어졌기 때문에 서버의 비정상적인 상태(잘못된 응답 형식, 허용되지 않은 응답 데이터, 시간 초과 등)을 판별하기엔 적합했지만 API 스펙의 세세한 부분을 정확히 검증하기에는 표현에 제한이 있었고, 로그인 후 쿠키를 전달한다던지 각 테스트 케이스에서 발생한 값들을 변수로 재활용 할 수 있게 하는 컨텍스트 관리 등의 기능을 제공하지 않고 있었습니다.
